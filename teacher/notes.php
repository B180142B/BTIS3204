...
<?php for ($i=1; $i <= 5; $i++) { ?>
                            ...
                            <dd>  
                            <div style="margin-left: 25px;"><h4><a href="../assets/notes/textbooks/<?php echo $i; ?>.pdf" target="_blank">Buku Teks</a></h4></div>

    <?php
    // Prepare a select statement
    $sql = "SELECT * FROM note LEFT JOIN teacher ON note.TeacherID = teacher.TeacherID WHERE NoteID > 5 AND ChapterID = ".$i." ORDER BY ChapterID ASC;";

    $res = mysqli_query($conn, $sql);

    while ($myrow = mysqli_fetch_array($res)) { ?>
                            <div style="margin-left: 25px;"><h4><a href="<?php echo $myrow['Link']; ?>" target="_blank"><?php echo $myrow['Topic']; ?> (<?php echo $myrow['Name']; ?>)</a></h4></div>
    <?php } ?>

                            <a href="create-notes.php?t=<?php echo $i; ?>" class="btn btn-medium btn-theme-border-color">Add Notes</a>
                            </dd>
<?php } ?>
