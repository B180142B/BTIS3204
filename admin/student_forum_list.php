...
<?php
// Prepare a select statement
$sql1 = "SELECT * FROM post LEFT JOIN student ON post.Student = student.StudentID ORDER BY DateTime DESC;";

$res1 = mysqli_query($conn, $sql1);

while ($myrow1 = mysqli_fetch_array($res1)) {?>
						<tr>
							<td style="color: <?php if($myrow1['Showed'] == 1){ echo "purple";} else { echo "grey";}?>; text-align: left; padding-left: 10px;"><b><?php echo $myrow1['Post']; ?></b></td>
							<td><?php echo $myrow1['Username']; ?><br><?php echo $myrow1['Name']; ?></td>
							<td><?php echo $myrow1['DateTime']; ?></td>
							<td><a href="comment.php?p=<?php echo $myrow1['PostID']; ?>">View comments</a><br><?php if($myrow1['Showed'] == 1){?><a href="delete_post.php?p=<?php echo $myrow1['PostID']; ?>" onclick="return confirm('Do you confirm to delete the post?')">Delete</a><?php } else {?><a href="undelete_post.php?p=<?php echo $myrow1['PostID']; ?>">Undelete</a><?php }?></td>
<?php } ?>
						</tr>
      				</table>
      			</div>
